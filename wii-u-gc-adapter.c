...
static int gpio_init(void)
{
    ...
    if (!gpio_chip) {
        fprintf(stderr, "Could not open chip.\n");
        return 1;
    }
    ...
    if (0 != gpiod_line_settings_set_bias(gpio_line_settings, GPIOD_LINE_BIAS_PULL_UP)) {
        fprintf(stderr, "Could not set line bias.\n");
...
static bool gpio_is_select_pressed(void)
{
    ...
    if (GPIOD_LINE_VALUE_ERROR == val) {
        ...
        return false;
    } else if (GPIOD_LINE_VALUE_ACTIVE == val) {
        return true;
    } else {
        return false;
    }
...
static bool uinput_create(int i, struct ports *port, unsigned char type)
{
    ...
    // support select on first controller
    if (i == 0) {
...
static void *adapter_thread(void *data)
{
    ...
    while (!a->quitting)
    {
        ...
        {
            // insert select state into first controller
            if (i == 0) {
                uint16_t btns = (uint16_t) controller[1] << 8 | (uint16_t) controller[2];
                bool select_pressed = gpio_is_select_pressed();
                uint16_t value = 1 << BUTTON_OFFSET_SELECT_INDEX;
                if (select_pressed) {
                    btns |= value;
                } else {
                    btns &= ~value;
                }
                controller[1] = (uint8_t)(btns >> 8);
                controller[2] = (uint8_t)(btns & 0xff);
            }
